...
async def demonstrate_fixed_features():
    ...
    try:
        ...
        if isinstance(result, dict) and 'token_count' in result:
            print_success(f"Contagem funcionando - {result['token_count']} tokens para {len(test_text)} caracteres")
        else:
    ...
    try:
        ...
        if isinstance(health, dict):
            ...
            if 'providers' in health:
                available = sum(1 for p in health['providers'].values() if p.get('available'))
