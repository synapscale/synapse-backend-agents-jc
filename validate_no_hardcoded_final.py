    ...
    for file in python_files:
        ...
        for ignore_pattern in ignore_patterns:
            if file.endswith('.example') or file.endswith('.template'):
                should_ignore = True
                break
            if 'venv' in file or '__pycache__' in file or '.git' in file or '_temp.py' in file:
                should_ignore = True
                break
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                for pattern_name, pattern in patterns.items():
                    ...
                    if matches:
                        ...
                        findings.append((file_path, pattern_name, line_num, line.strip()))
...
def is_valid_usage(line: str, pattern_name: str, file_path: str) -> bool:
    ...
    if pattern_name == 'secret_assignments':
        ...
        if 'Field(' in line and ('config' in file_path.lower() or 'settings' in file_path.lower()):
            return True
    ...
    if pattern_name == 'base64_keys':
        ...
        if 'generate' in line.lower() or 'hash' in line.lower() or 'random' in line.lower():
            return True
        ...
        try:
            ...
            for key_name, pattern in required_patterns:
                if key_name in content and not re.search(pattern, content):
                    ...
                    if f'settings.{key_name}' not in content and f'settings.JWT_{key_name}' not in content:
                        issues.append(f"{file_path}: {key_name} pode não estar usando .env corretamente")
