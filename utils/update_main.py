...
if 'from src.synapse.models.file import File' not in content:
    lines = content.split('\n')
    # Encontrar onde adicionar o import
    for i, line in enumerate(lines):
        if line.startswith('from src.synapse.') and 'import' in line:
            # Adicionar após outros imports do synapse
            lines.insert(i + 1, 'from src.synapse.models.file import File  # Garantir registro do modelo')
            break
