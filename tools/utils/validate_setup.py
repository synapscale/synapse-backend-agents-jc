...
class SynapScaleValidator:
    ...
    def check_env_variables(self) -> bool:
        ...
        if missing_critical:
            self.errors.append(f"Variáveis críticas não configuradas: {', '.join(missing_critical)}")
            print(f"❌ Variáveis críticas não configuradas: {', '.join(missing_critical)}")
            return False
    ...
    def check_import_main(self) -> bool:
        ...
        try:
            result = subprocess.run([
                str(python_exe), "-c", 
                "import sys; sys.path.append('.'); from src.synapse.main import app; print('✅ Import OK')"
    ...
    def check_server_startup(self) -> bool:
        ...
        try:
            process = subprocess.Popen([
                ...
                "--host", "127.0.0.1", 
                "--port", "8001"
            ], cwd=self.root_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
'''
        ...
        try:
            ...
            ], capture_output=True, text=True, timeout=15, cwd=self.root_path)
    ...
    def run_validation(self) -> bool:
        ...
        print(f"✅ Testes passaram: {passed_checks}/{total_checks} ({success_rate:.1f}%)")
