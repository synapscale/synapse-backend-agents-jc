...
async def list_llms(
    ...
    conditions.append(or_(LLM.is_public == True, LLM.user_id == current_user.id))
    ...
    if search:
        ...
        conditions.append(
            or_(
                ...
                LLM.model_id.ilike(search_term),
            )
        )
    ...
    if is_public is not None:
        conditions.append(LLM.is_public == is_public)
...
async def list_llm_conversations(
    ...
    llm_result = await db.execute(
        select(LLM).where(
            and_(
                LLM.id == llm_id,
                or_(LLM.is_public == True, LLM.user_id == current_user.id),
            )
        )
    )
