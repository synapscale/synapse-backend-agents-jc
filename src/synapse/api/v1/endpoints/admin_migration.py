...
from synapse.middlewares.legacy_tracking import get_current_migration_phase, get_migration_phase_info

logger = logging.getLogger(__name__)
router = APIRouter()


@router.get("/migration/dashboard", summary="Dashboard de Migração LLM", tags=["admin", "migration"])
async def get_migration_dashboard(
    ...
    try:
        ...
        logger.error(f"Erro ao gerar dashboard de migração para admin {current_admin.id}: {e}")
        raise HTTPException(status_code=500, detail="Erro interno do servidor")


@router.get("/migration/users", summary="Usuários em Migração", tags=["admin", "migration"])
async def get_migration_users(
    ...
    """
    try:
        logger.info(f"Admin {current_admin.id} listando usuários de migração - status: {status}")
        ...
        logger.error(f"Erro ao listar usuários de migração: {e}")
        raise HTTPException(status_code=500, detail="Erro interno do servidor")


@router.post("/migration/communications/send", summary="Enviar Comunicação de Migração", tags=["admin", "migration"])
async def send_migration_communication(
    target_group: str = Query(..., regex="^(all|legacy|migrating|enterprise|developers)$"),
    ...
    """
    try:
        logger.info(f"Admin {current_admin.id} enviando comunicação de migração - grupo: {target_group}, tipo: {communication_type}")
        ...
        communication_result = {
            ...
            "message_preview": custom_message[:100] if custom_message else _get_default_message(communication_type),
        ...
        logger.info(f"Comunicação de migração enviada por admin {current_admin.id}: {communication_result['communication_id']}")
        
        return communication_result
        
    except Exception as e:
        logger.error(f"Erro ao enviar comunicação de migração: {e}")
        raise HTTPException(status_code=500, detail="Erro interno do servidor")


@router.get("/migration/analytics", summary="Analytics de Migração", tags=["admin", "migration"])
async def get_migration_analytics(
    ...
    try:
        ...
        logger.info(f"Admin {current_admin.id} solicitando analytics de migração - período: {start_date} a {end_date}")
        ...
        analytics_data = {
            ...
            "migration_trends": await _get_migration_trends(db, start_date, end_date, granularity),
            "endpoint_usage_evolution": await _get_endpoint_usage_evolution(db, start_date, end_date, granularity),
            "user_migration_patterns": await _get_user_migration_patterns(db, start_date, end_date),
            "communication_effectiveness": await _get_communication_effectiveness(db, start_date, end_date),
            "support_impact": await _get_support_impact_analysis(db, start_date, end_date)
        }
        ...
        logger.error(f"Erro ao gerar analytics de migração: {e}")
        raise HTTPException(status_code=500, detail="Erro interno do servidor")
...
async def _get_most_used_legacy_endpoints(db: Session) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {"endpoint": "/api/v1/anthropic/generate", "usage_count": 1920, "unique_users": 89},
        ...
        {"endpoint": "/api/v1/anthropic/models", "usage_count": 670, "unique_users": 45}
    ]
...
async def _get_top_legacy_users(db: Session) -> List[Dict[str, Any]]:
    ...
    # Simulação
    return [
        {"user_id": 1001, "email": "heavy.user@company.com", "legacy_requests": 450, "last_legacy_use": "2024-12-09T08:30:00Z"},
        {"user_id": 1042, "email": "api.client@enterprise.com", "legacy_requests": 320, "last_legacy_use": "2024-12-09T09:15:00Z"},
        {"user_id": 1089, "email": "dev.team@startup.io", "legacy_requests": 280, "last_legacy_use": "2024-12-09T07:45:00Z"},
        {"user_id": 1156, "email": "integration@bigcorp.com", "legacy_requests": 190, "last_legacy_use": "2024-12-09T06:30:00Z"}
    ]
...
async def _get_users_by_migration_status(db: Session, status: str, limit: int, offset: int) -> Dict[str, Any]:
    # Simulação simplificada
    total_users = await _get_total_users(db)
    
    if status == "legacy":
        users = [{"id": i+1000, "email": f"legacy.user{i}@example.com", "last_legacy_use": "2024-12-09"} for i in range(limit)]
        total = await _get_legacy_users_count(db)
    elif status == "migrated": 
        users = [{"id": i+2000, "email": f"migrated.user{i}@example.com", "migration_date": "2024-12-01"} for i in range(limit)]
        total = await _get_migrated_users_count(db)
    else:
        users = [{"id": i+3000, "email": f"user{i}@example.com", "status": "migrating"} for i in range(limit)]
        total = total_users
...
async def _get_users_for_communication(db: Session, target_group: str) -> List[Dict[str, Any]]:
    # Simulação
...
async def _get_migration_trends(db: Session, start_date: datetime, end_date: datetime, granularity: str) -> List[Dict[str, Any]]:
    return []  # Implementar simulação de trends

async def _get_endpoint_usage_evolution(db: Session, start_date: datetime, end_date: datetime, granularity: str) -> List[Dict[str, Any]]:
    return []  # Implementar simulação de evolução

async def _get_user_migration_patterns(db: Session, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    return {}  # Implementar simulação de padrões

async def _get_communication_effectiveness(db: Session, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    return {}  # Implementar simulação de efetividade

async def _get_support_impact_analysis(db: Session, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
