...
from synapse.middlewares.legacy_tracking import (
    get_current_migration_phase,
...
@router.get("/migration/dashboard", summary="Dashboard de Migração LLM")
async def get_migration_dashboard(
    ...
    try:
        ...
        logger.error(
            f"Erro ao gerar dashboard de migração para admin {current_admin.id}: {e}"
        )
        raise HTTPException(status_code=500, detail="Erro interno do servidor")


@router.get("/migration/users", summary="Usuários em Migração")
...
@router.get("/migration/analytics", summary="Analytics de Migração")
