...
async def list_nodes(
    ...
    return NodeListResponse(
        items=nodes, total=total, page=page, size=size, pages=(total + size - 1) // size
    )
...
async def get_node(
    ...
    if not node:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Node não encontrado"
        )
...
async def list_node_categories(db: Session = Depends(get_db)):
    """Listar categorias de nodes disponíveis"""
    categories = (
        db.query(Node.category)
        .filter(Node.category.isnot(None), Node.is_public == True)
        .distinct()
        .all()
    )
...
@router.get("/types/")
async def list_node_types(db: Session = Depends(get_db)):
    """Listar tipos de nodes disponíveis"""
    types = (
        db.query(Node.type)
        .filter(Node.type.isnot(None), Node.is_public == True)
        .distinct()
        .all()
    )

    return {"types": [type_[0] for type_ in types if type_[0]]}
