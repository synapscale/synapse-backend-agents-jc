"""
Endpoints for managing CustomReports.
"""
...
from synapse.schemas.custom_report import (
    CustomReportCreate,
    CustomReportUpdate,
    CustomReportResponse,
    CustomReportListResponse,
)
from synapse.models import CustomReport, User
...
async def create_custom_report(
    ...
    db_item = CustomReport(**create_data)
    db.add(db_item)
    await db.commit()
...
@router.get("/{item_id}", response_model=CustomReportResponse)
async def get_custom_report(
    item_id: uuid.UUID,
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_active_user),
):
    """Get a specific custom report by its ID."""
    result = await db.execute(
        select(CustomReport).where(CustomReport.id == item_id, CustomReport.tenant_id == current_user.tenant_id)
    )
...
async def list_custom_reports(
    ...
    query = select(CustomReport).where(CustomReport.tenant_id == current_user.tenant_id)
...
@router.put("/{item_id}", response_model=CustomReportResponse)
async def update_custom_report(
    item_id: uuid.UUID,
    item_in: CustomReportUpdate,
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_active_user),
):
