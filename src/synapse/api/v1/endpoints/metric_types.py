"""
Endpoints for managing MetricTypes.
"""
...
from synapse.schemas.metric_type import (
    MetricTypeCreate,
    MetricTypeUpdate,
    MetricTypeResponse,
    MetricTypeListResponse,
)
from synapse.models import MetricType, User
...
async def create_metric_type(
    ...
    db_item = MetricType(**create_data)
    db.add(db_item)
    await db.commit()
...
@router.get("/{item_id}", response_model=MetricTypeResponse)
async def get_metric_type(
    item_id: uuid.UUID,
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_active_user),
):
    """Get a specific metric type by its ID."""
    result = await db.execute(
        select(MetricType).where(MetricType.id == item_id)
    )
    ...
    if not item:
        raise HTTPException(
            ...
            detail="MetricType not found"
        )
    return item


@router.get("/", response_model=MetricTypeListResponse)
...
@router.put("/{item_id}", response_model=MetricTypeResponse)
async def update_metric_type(
    item_id: uuid.UUID,
    item_in: MetricTypeUpdate,
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_active_user),
):
