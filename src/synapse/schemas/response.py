...
class DataResponse(BaseResponse, Generic[T]):
    ...
    data: T = Field(
        ...,
        description="Os dados da resposta"
    )
...
class ListResponse(BaseResponse, Generic[T]):
    ...
    data: List[T] = Field(
        ...,
        description="Lista de itens da resposta"
    )
    total: Optional[int] = Field(
        default=None,
        ge=0,
        description="Número total de itens (opcional)"
    )
    pagination: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Metadados de paginação (opcional)"
    )
...
class GenericBulkOperationResponse(BaseResponse):
    ...
    results: Dict[str, Any] = Field(
        ...,
        description="Resultados da operação em lote"
    )
...
def create_pagination_meta(
    ...
    return PaginationMeta(
        ...
        last_page_url=f"{base_url}?page={total_pages}&size={page_size}" if base_url else None,
        next_page_url=f"{base_url}?page={current_page + 1}&size={page_size}" if base_url and current_page < total_pages else None,
        previous_page_url=f"{base_url}?page={current_page - 1}&size={page_size}" if base_url and current_page > 1 else None
    )
...
def wrap_empty_response(message: str = "Operation completed successfully") -> Dict[str, Any]:
    """
...
def convert_legacy_response(legacy_data: Any, response_type: str = "data") -> Dict[str, Any]:
    """
...
def add_request_context(response_data: Dict[str, Any], request: Request) -> Dict[str, Any]:
    """
...
def standardize_endpoint_response(
