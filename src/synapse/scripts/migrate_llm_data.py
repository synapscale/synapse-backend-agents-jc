...
def analyze_migration_gaps(db_session: Session) -> MigrationReport:
    ...
    for enum_model in enum_models:
        matching_db_models = [
            db_model for db_model in db_models 
            if db_model["name"] == enum_model.model_id and db_model["provider"] == enum_model.provider
        ]
...
def migrate_enum_models_to_database(db_session: Session, report: MigrationReport) -> int:
...
def sync_enums_to_database(db_session: Session) -> bool:
    ...
    for enum_model in report.enum_models:
        db_model = db_session.query(LLM).filter(
            LLM.name == enum_model.model_id,
            LLM.provider == enum_model.provider
        ).first()
...
def generate_migration_report(db_session: Session, output_file: Optional[str] = None) -> str:
...
"""
    ...
    if report.missing_in_db:
        report_content += f"1. Run migration to add {len(report.missing_in_db)} missing models\n"
...
def sync_enums(force):
    ...
    try:
        ...
        if changes_needed == 0:
            click.echo("✅ No synchronization needed - enums and database are already consistent")
            return
