...
class Settings(BaseSettings):
    ...
    DESCRIPTION: str = Field(
        default_factory=lambda: os.getenv("DESCRIPTION", "Plataforma de Automação com IA"),
    ...
    SERVER_PORT: int = Field(
        default_factory=lambda: int(os.getenv("SERVER_PORT", os.getenv("PORT", "8000"))),
    ...
    BACKEND_CORS_ORIGINS: str = Field(
        default_factory=lambda: os.getenv("BACKEND_CORS_ORIGINS", '["http://localhost:3000"]'),
    ...
    CORS_ALLOW_METHODS: list[str] = Field(
        default_factory=lambda: _parse_list_env(os.getenv("CORS_ALLOW_METHODS", "GET,POST,PUT,DELETE,OPTIONS,PATCH")),
    ...
    EMAIL_NOTIFICATIONS_ENABLED: bool = Field(
        default_factory=lambda: os.getenv("EMAIL_NOTIFICATIONS_ENABLED", "True").lower() == "true",
    ...
    ALLOWED_EXTENSIONS: str = Field(
        default_factory=lambda: os.getenv("ALLOWED_EXTENSIONS", ".pdf,.doc,.docx,.txt,.csv"),
        description="Extensões permitidas"
    )
    ALLOWED_FILE_EXTENSIONS: list[str] = Field(
        default_factory=lambda: _parse_list_env(os.getenv("ALLOWED_FILE_EXTENSIONS", ".pdf,.doc,.docx,.txt,.csv")),
    ...
    WEBSOCKET_ENABLED: bool = Field(
        default_factory=lambda: os.getenv("WEBSOCKET_ENABLED", "True").lower() == "true",
    ...
    CENTRALIZED_LOG_LABELS: str = Field(
        default_factory=lambda: os.getenv("CENTRALIZED_LOG_LABELS", '{"service":"synapscale","environment":"development"}'),
        description="Labels para logs centralizados"
    )
    ENABLE_LOG_FILE_OUTPUT: bool = Field(
        default_factory=lambda: os.getenv("ENABLE_LOG_FILE_OUTPUT", "True").lower() == "true",
    ...
    MARKETPLACE_ENABLED: bool = Field(
        default_factory=lambda: os.getenv("MARKETPLACE_ENABLED", "True").lower() == "true",
        description="Habilitar marketplace"
    )
    MARKETPLACE_APPROVAL_REQUIRED: bool = Field(
        default_factory=lambda: os.getenv("MARKETPLACE_APPROVAL_REQUIRED", "True").lower() == "true",
    ...
    NOTIFICATIONS_ENABLED: bool = Field(
        default_factory=lambda: os.getenv("NOTIFICATIONS_ENABLED", "True").lower() == "true",
        description="Habilitar notificações"
    )
    PUSH_NOTIFICATIONS_ENABLED: bool = Field(
        default_factory=lambda: os.getenv("PUSH_NOTIFICATIONS_ENABLED", "False").lower() == "true",
    ...
    ANALYTICS_ENABLED: bool = Field(
        default_factory=lambda: os.getenv("ANALYTICS_ENABLED", "True").lower() == "true",
    ...
    PROMETHEUS_ENABLED: bool = Field(
        default_factory=lambda: os.getenv("PROMETHEUS_ENABLED", "False").lower() == "true",
        description="Habilitar Prometheus"
    )
    HEALTH_CHECK_ENABLED: bool = Field(
        default_factory=lambda: os.getenv("HEALTH_CHECK_ENABLED", "True").lower() == "true",
    ...
    def validate_openai_config(self) -> dict[str, Any]:
        ...
        if self.OPENAI_API_TYPE == "azure":
            if not self.OPENAI_API_BASE or not self.OPENAI_API_VERSION:
                errors.append("Para Azure OpenAI, OPENAI_API_BASE e OPENAI_API_VERSION são obrigatórios")
                return {
                    "valid": False,
                    "errors": errors,
                    "warnings": warnings,
    ...
    def get_openai_client_config(self) -> dict[str, Any]:
        ...
        if not validation_result["valid"]:
            raise ValueError(f"Configuração OpenAI inválida: {validation_result['errors']}")
...
def validate_settings():
    ...
    if settings.STORAGE_TYPE == "s3":
        if not all([settings.AWS_ACCESS_KEY_ID, settings.AWS_SECRET_ACCESS_KEY, settings.AWS_BUCKET_NAME]):
            raise ValueError("Para STORAGE_TYPE=s3, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY e AWS_BUCKET_NAME são obrigatórios")
    
    if settings.STORAGE_TYPE == "gcs":
        if not all([settings.GCS_BUCKET_NAME, settings.GCS_CREDENTIALS_PATH]):
            raise ValueError("Para STORAGE_TYPE=gcs, GCS_BUCKET_NAME e GCS_CREDENTIALS_PATH são obrigatórios")
    
    if settings.EMAIL_NOTIFICATIONS_ENABLED:
        if not all([settings.SMTP_HOST, settings.SMTP_PORT, settings.SMTP_USERNAME, settings.SMTP_PASSWORD]):
            raise ValueError("Para email habilitado, configurações SMTP são obrigatórias")
