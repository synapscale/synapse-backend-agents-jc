...
class LLMExecutor(BaseExecutor):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        raise Exception(f"Erro Anthropic {response.status}: {error_text}")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(endpoint, json=payload) as response:
                    if response.status != 200:
                        error_text = await response.text()
                        raise Exception(f"Erro modelo local {response.status}: {error_text}")
    ...
    def _calculate_cost(
        ...
        prompt_cost = (token_usage.get("prompt_tokens", 0) / 1000) * model_pricing["prompt"]
        completion_cost = (token_usage.get("completion_tokens", 0) / 1000) * model_pricing["completion"]
