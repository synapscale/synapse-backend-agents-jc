...
from src.synapse.models.workflow_execution import NodeExecution, NodeExecutionStatus
from src.synapse.models.node import Node
from src.synapse.schemas.workflow_execution import NodeExecutionUpdate
...
class BaseExecutor(ABC):
    ...
    def extract_inputs_from_connections(
        ...
        if hasattr(node, 'config') and node.config:
            config = node.config if isinstance(node.config, dict) else json.loads(node.config)
                ...
                for input_name, input_config in config["inputs"].items():
                    if isinstance(input_config, dict) and "source_node" in input_config:
                        ...
                        if source_output is not None:
                            if isinstance(source_output, dict) and source_output_key in source_output:
    ...
    async def execute_with_retry(
        ...
        for attempt in range(max_retries + 1):
            try:
                if attempt > 0:
                    self.logger.info(f"Tentativa {attempt + 1}/{max_retries + 1} para nó {node.node_id}")
                ...
                self.logger.warning(f"Erro na tentativa {attempt + 1} para nó {node.node_id}: {str(e)}")
