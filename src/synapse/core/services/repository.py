...
class BaseRepository(IRepository[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def update(
        ...
        try:
            ...
            await self.db.rollback()
            self.logger.error(f"Failed to update {self.model.__name__} with ID {id}: {e}")
    ...
    async def delete(self, id: Union[UUID, str, int]) -> bool:
        ...
        try:
            ...
            await self.db.rollback()
            self.logger.error(f"Failed to delete {self.model.__name__} with ID {id}: {e}")
    ...
    async def exists(self, id: Union[UUID, str, int]) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to check existence of {self.model.__name__} with ID {id}: {e}")
            raise DatabaseError(f"Failed to check existence of {self.model.__name__}") from e
    ...
    async def get_with_relations(
        ...
        try:
            ...
            self.logger.error(f"Failed to get {self.model.__name__} with relations: {e}")
            raise DatabaseError(f"Failed to get {self.model.__name__} with relations") from e
