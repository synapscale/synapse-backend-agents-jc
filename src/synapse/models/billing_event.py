...
class BillingEvent(Base):
    ...
    event_type = Column(String(50), nullable=False, index=True)  # usage, subscription, credit, refund
    amount_usd = Column(Float, nullable=False)
    description = Column(Text, nullable=True)

    # Relacionamentos com outras entidades
    related_usage_log_id = Column(UUID(as_uuid=True), ForeignKey("synapscale_db.llms_usage_logs.id", ondelete="SET NULL"), nullable=True)
    related_message_id = Column(UUID(as_uuid=True), ForeignKey("synapscale_db.llms_messages.id", ondelete="SET NULL"), nullable=True)
    ...
    # Status do evento
    status = Column(String(20), server_default='pending', index=True)  # pending, completed, failed, refunded
    ...
    def to_dict(self):
        ...
        return {
            ...
            "related_usage_log_id": str(self.related_usage_log_id) if self.related_usage_log_id else None,
