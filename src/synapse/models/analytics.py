...
class AnalyticsEvent(Base):
    ...
    # Dados do evento
    properties = Column(JSON, default=dict)  # Propriedades específicas do evento
...
class SystemPerformanceMetric(Base):
    ...
    # Contexto
    service = Column(String(50), nullable=False, index=True)  # api, frontend, executor, etc.
...
class CustomReport(Base):
    ...
    def __repr__(self):
        return f"<CustomReport(id={self.id}, name='{self.name}', user_id={self.user_id})>"
...
class ReportExecution(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id"), index=True)  # Quem executou (pode ser automático)
...
class UserInsight(Base):
    ...
    insight_type = Column(String(50), nullable=False, index=True)  # performance, usage, recommendation, etc.
    category = Column(String(50), nullable=False, index=True)
    ...
    is_evergreen = Column(Boolean, default=False, nullable=False)  # Insight sempre relevante
