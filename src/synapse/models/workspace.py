...
class WorkspaceMember(Base):
    ...
    # Status
    status = Column(String(20), default="active", nullable=False)  # active, suspended, left
    ...
    def has_permission(self, permission: str, resource_type: str = None) -> bool:
        ...
        default_permissions = {
            WorkspaceRole.EDITOR: {
                "read_projects", "write_projects", "create_projects",
                "read_members", "comment", "chat"
            },
            WorkspaceRole.VIEWER: {
                "read_projects", "read_members", "comment"
            },
            WorkspaceRole.GUEST: {
                "read_projects"
            }
...
class ProjectCollaborator(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    project_id = Column(Integer, ForeignKey("workspace_projects.id"), nullable=False, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
...
class WorkspaceInvitation(Base):
    ...
    invited_user_id = Column(Integer, ForeignKey("users.id"), index=True)  # Se usuário já existe
...
class WorkspaceActivity(Base):
    ...
    # Atividade
    action = Column(String(50), nullable=False, index=True)  # created, updated, deleted, etc.
...
class ProjectComment(Base):
    ...
    project_id = Column(Integer, ForeignKey("workspace_projects.id"), nullable=False, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    parent_id = Column(Integer, ForeignKey("project_comments.id"), index=True)  # Para threads
