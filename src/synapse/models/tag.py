...
from sqlalchemy import Column, String, Boolean, DateTime, Text, JSON, Float, ForeignKey, text
...
class Tag(Base):
    ...
    target_type = Column(String(50), nullable=False, index=True)  # conversation, message, user, workspace
    ...
    # Configurações
    is_system_tag = Column(Boolean, server_default=text('false'), index=True)  # tag do sistema vs usuário
    created_by_user_id = Column(UUID(as_uuid=True), ForeignKey("synapscale_db.users.id", ondelete="SET NULL"), nullable=True, index=True)
