...
from synapse.models.marketplace import (
...
class MarketplaceService:
    ...
    def search_components(self, search_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Construir a consulta base - usando status ao invés de is_active
            db_query = self.db.query(MarketplaceComponent).filter(
                MarketplaceComponent.status.in_(["approved", "published"])
            )
            ...
            if sort_order.lower() == "desc":
                db_query = db_query.order_by(
                    desc(getattr(MarketplaceComponent, sort_by))
                )
            else:
                db_query = db_query.order_by(
                    asc(getattr(MarketplaceComponent, sort_by))
                )
    ...
    def get_component_by_id(self, component_id: str) -> Optional[MarketplaceComponent]:
        ...
        try:
            component = (
                self.db.query(MarketplaceComponent)
                .filter(MarketplaceComponent.id == component_id)
                .first()
            )
            if component:
                self.db.refresh(component)
        ...
        try:
            component = MarketplaceComponent(
                name=(
                    ...
                    else "Novo Componente"
                ),
                title=(
                    component_data.title
                    if hasattr(component_data, "title")
                    else component_data.name if hasattr(component_data, "name") else "Novo Componente"
                ),
                description=(
                ...
                component_type=(
                    ...
                    if hasattr(component_data, "component_type")
                    else "workflow"
                ),
                author_id=user_id,
                created_at=datetime.utcnow(),
                status="pending",  # Campo correto do banco
                version="1.0.0",
                license_type="MIT",
                is_free=True,
                rating_average=0.0,  # Campo correto do banco
                rating_count=0,     # Campo correto do banco
                downloads_count=0,  # Campo correto do banco
                install_count=0,
                view_count=0,
                like_count=0,
    ...
    def update_component(self, component_id: int, component_data, user_id: int) -> dict:
        ...
        try:
            ...
            return {"id": component.id, "updated_at": component.updated_at.isoformat()}
        except Exception as e:
            self.db.rollback()
    ...
    def download_component(
        self, component_id: int, user_id: int, version: str = None
    ) -> dict:
    ...
    def install_component(
        self, component_id: int, user_id: int, workspace_id: int = None
    ) -> dict:
    ...
    def get_rating_stats(self, component_id: int) -> dict:
        ...
        return {
            "total_ratings": 25,
            "average_rating": 4.2,
            "rating_distribution": {"5": 12, "4": 8, "3": 3, "2": 1, "1": 1},
        }

    def update_rating(self, rating_id: int, rating_data, user_id: int) -> dict:
        """Atualiza uma avaliação"""
        return {"id": rating_id, "updated_at": datetime.utcnow().isoformat()}
    ...
    def mark_rating_helpful(self, rating_id: int, user_id: int) -> dict:
        ...
        return {"rating_id": rating_id, "helpful": True}

    def purchase_component(
        self, component_id: int, purchase_data, user_id: int
    ) -> dict:
    ...
    def get_component_metadata(self, component_id: str) -> dict:
        """Obtém metadados de um componente incluindo informações da versão atual"""
        try:
            ...
            logger.error(f"Erro ao buscar metadados do componente {component_id}: {e}")
            return {}

    def create_component_version(
        self, component_id: int, version: str, file_data, user_id: int
    ) -> dict:
    ...
    def favorite_component(self, component_id: int, user_id: int) -> dict:
        ...
        return {"component_id": component_id, "favorited": True}

    def unfavorite_component(self, component_id: int, user_id: int) -> dict:
        """Remove componente dos favoritos"""
        return {"component_id": component_id, "favorited": False}
    ...
    def bulk_component_operation(self, operation, user_id: int) -> dict:
        ...
        return {"operation": str(operation), "processed": 0, "success": 0, "failed": 0}

    def feature_component(
        self, component_id: int, featured: bool, user_id: int
    ) -> dict:
        """Destaca um componente"""
        return {"component_id": component_id, "featured": featured}
    ...
    def get_top_components_report(
        self, metric: str = "downloads", limit: int = 10
    ) -> dict:
        """Relatório de top componentes"""
        return {"metric": metric, "components": [], "period": "last_30_days"}
