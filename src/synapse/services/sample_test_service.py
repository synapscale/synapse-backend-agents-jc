...
class SampleTestService(BaseService[User, UserCreateRequest, UserUpdateRequest]):
    ...
    async def create_user(self, user_data: UserCreateRequest) -> UserResponse:
        ...
        try:
            ...
            if existing_username:
                raise ServiceError(f"User with username {user_data.username} already exists")
    ...
    async def update_user(self, user_id: UUID, update_data: UserUpdateRequest) -> Optional[UserResponse]:
        """
        ...
        try:
            ...
            if update_data.username and update_data.username != user.username:
                existing = await self.user_repository.find_by_username(update_data.username)
                if existing:
                    raise ServiceError(f"Username {update_data.username} already in use")
        ...
        try:
            async with UnitOfWork(self.session) as uow:
                ...
                stats = {
                    ...
                    "activation_rate": round((active_count / total_users * 100), 2) if total_users > 0 else 0,
