...
class WorkspaceMemberService:
    ...
    def add_member(
        ...
        try:
            ...
            # 1. VALIDAÇÕES
            validation = self._validate_member_addition(workspace_id, user_id, invited_by_user_id)
            ...
            logger.info(f"✅ Membro {user_id} adicionado com sucesso ao workspace {workspace_id}")
    ...
    def remove_member(
        ...
        try:
            ...
            # 1. VALIDAÇÕES
            validation = self._validate_member_removal(workspace_id, user_id, removed_by_user_id)
    ...
    def update_member_role(
        ...
        """
        try:
            logger.info(f"Atualizando role do membro {user_id} no workspace {workspace_id}")

            # 1. VALIDAÇÕES
            validation = self._validate_role_update(workspace_id, user_id, new_role, updated_by_user_id)
    ...
    def _validate_member_addition(self, workspace_id: str, user_id: str, invited_by_user_id: str) -> Dict[str, Any]:
        """Validações para adição de membro"""
        
        # 1. Verificar se workspace existe
        workspace = self.db.query(Workspace).filter(Workspace.id == workspace_id).first()
        if not workspace:
            return {"can_add": False, "reason": "Workspace não encontrado", "error_code": "WORKSPACE_NOT_FOUND"}
        ...
        if not new_member_user or not inviter_user:
            return {"can_add": False, "reason": "Usuário não encontrado", "error_code": "USER_NOT_FOUND"}
    ...
    def _validate_member_removal(self, workspace_id: str, user_id: str, removed_by_user_id: str) -> Dict[str, Any]:
        """Validações para remoção de membro"""
        
        # 1. Verificar se workspace existe
        workspace = self.db.query(Workspace).filter(Workspace.id == workspace_id).first()
        if not workspace:
            return {"can_remove": False, "reason": "Workspace não encontrado", "error_code": "WORKSPACE_NOT_FOUND"}
        ...
        if not member:
            return {"can_remove": False, "reason": "Membro não encontrado", "error_code": "MEMBER_NOT_FOUND"}
        ...
        if not remover_member:
            return {"can_remove": False, "reason": "Sem permissão para remover membros", "error_code": "NO_PERMISSION"}

        # Apenas owners e admins podem remover, ou o próprio usuário pode sair
        if (remover_member.role not in [WorkspaceRole.OWNER, WorkspaceRole.ADMIN] and 
    ...
    def _validate_role_update(
        ...
        workspace = self.db.query(Workspace).filter(Workspace.id == workspace_id).first()
        if not workspace:
            return {"can_update": False, "reason": "Workspace não encontrado", "error_code": "WORKSPACE_NOT_FOUND"}
        ...
        if not member:
            return {"can_update": False, "reason": "Membro não encontrado", "error_code": "MEMBER_NOT_FOUND"}
