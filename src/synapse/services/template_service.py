...
from src.synapse.models.template import (
    ...
    TemplateCollection, TemplateUsage, TemplateCategory, TemplateStatus, TemplateLicense
)
...
class TemplateService:
    ...
    async def get_template(
        ...
        try:
            ...
            recent_downloads = db.query(TemplateDownload).filter(
                TemplateDownload.template_id == template.id,
                TemplateDownload.downloaded_at >= thirty_days_ago
            ).count()
            
            recent_usage = db.query(TemplateUsage).filter(
            ...
            response_data.update({
                ...
                "author_avatar": getattr(template.author, 'avatar_url', None) if template.author else None,
    ...
    async def search_templates(
        ...
        try:
            ...
            query = query.filter(WorkflowTemplate.status == TemplateStatus.PUBLISHED.value)
            ...
            if filters.rating_min is not None:
                query = query.filter(WorkflowTemplate.rating_average >= filters.rating_min)
                
            if filters.complexity_min is not None:
                query = query.filter(WorkflowTemplate.complexity_level >= filters.complexity_min)
                
            if filters.complexity_max is not None:
                query = query.filter(WorkflowTemplate.complexity_level <= filters.complexity_max)
                
            if filters.is_featured is not None:
                query = query.filter(WorkflowTemplate.is_featured == filters.is_featured)
                
            if filters.is_verified is not None:
                query = query.filter(WorkflowTemplate.is_verified == filters.is_verified)
                
            if filters.author_id:
                query = query.filter(WorkflowTemplate.author_id == filters.author_id)
                
            if filters.created_after:
                query = query.filter(WorkflowTemplate.created_at >= filters.created_after)
                
            if filters.created_before:
                query = query.filter(WorkflowTemplate.created_at <= filters.created_before)
                
            if filters.industries:
                for industry in filters.industries:
                    query = query.filter(WorkflowTemplate.industries.contains([industry]))
                    
            if filters.use_cases:
                for use_case in filters.use_cases:
                    query = query.filter(WorkflowTemplate.use_cases.contains([use_case]))
    ...
    async def download_template(
        ...
        try:
            ...
            self.logger.info(f"✅ Download do template {template_id} registrado para usuário {user_id}")
            return True
    ...
    async def install_template(
        ...
        try:
            ...
            # Nome do workflow
            workflow_name = install_data.workflow_name or f"{template.title} (Instalado)"
            ...
            self.logger.info(f"✅ Template {template.template_id} instalado como workflow {new_workflow.id}")
    ...
    async def get_user_favorites(
        ...
        try:
            ...
            favorites = db.query(TemplateFavorite).options(
    ...
    async def get_template_stats(self, db: Session) -> TemplateStats:
        ...
        try:
            ...
            published = db.query(WorkflowTemplate).filter(
                WorkflowTemplate.status == TemplateStatus.PUBLISHED.value
            ).count()
            featured = db.query(WorkflowTemplate).filter(
                WorkflowTemplate.is_featured == True
            ).count()
            verified = db.query(WorkflowTemplate).filter(
                WorkflowTemplate.is_verified == True
            ).count()
            free = db.query(WorkflowTemplate).filter(
                WorkflowTemplate.license_type == TemplateLicense.FREE.value
            ).count()
            premium = db.query(WorkflowTemplate).filter(
            ...
            avg_rating = db.query(func.avg(WorkflowTemplate.rating_average)).scalar() or 0.0
            ...
            top_templates = db.query(WorkflowTemplate).filter(
                WorkflowTemplate.is_public == True
            ).order_by(
            ...
            # Templates recentes
            recent_templates = db.query(WorkflowTemplate).filter(
                WorkflowTemplate.is_public == True
            ).order_by(
            ...
            return TemplateStats(
                ...
                recent_templates=[TemplateResponse.from_orm(t) for t in recent_templates]
            )
    ...
    async def get_user_template_stats(self, db: Session, user_id: int) -> UserTemplateStats:
        """
        Obtém estatísticas de templates do usuário
        """
        try:
            created = db.query(WorkflowTemplate).filter(
                WorkflowTemplate.author_id == user_id
            ).count()
            ...
            total_downloads = db.query(func.sum(WorkflowTemplate.download_count)).filter(
                WorkflowTemplate.author_id == user_id
            ).scalar() or 0
            
            # Reviews dos templates do usuário
            total_reviews = db.query(TemplateReview).join(WorkflowTemplate).filter(
                WorkflowTemplate.author_id == user_id
            ).count()
            
            # Rating médio
            avg_rating = db.query(func.avg(WorkflowTemplate.rating_average)).filter(
                WorkflowTemplate.author_id == user_id
            ).scalar() or 0.0
            ...
            # Coleções do usuário
            collections = db.query(TemplateCollection).filter(
                TemplateCollection.creator_id == user_id
            ).count()
            
            # Ganhos (templates premium)
            earnings = db.query(func.sum(
