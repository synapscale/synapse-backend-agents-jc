...
class LegacyEndpointTrackingMiddleware(BaseHTTPMiddleware):
    ...
    def _is_legacy_endpoint(self, path: str) -> bool:
        ...
        return any(path.startswith(legacy_path.split('?')[0]) 
    ...
    def _create_redirect_response(self, request: Request) -> RedirectResponse:
        ...
        response = RedirectResponse(
            url=new_path,
            status_code=307  # Preserva método HTTP e body
        )
    ...
    def _add_deprecation_headers(self, response: Response, legacy_path: str):
        ...
        response.headers["Link"] = '</docs/llm-migration-guide>; rel="alternate"; title="Migration Guide"'
        ...
        response.headers["X-Migration-Phase"] = self.migration_phase
        response.headers["X-Migration-Guide"] = "https://docs.synapscale.com/llm-migration"
        response.headers["X-New-Endpoint"] = self.LEGACY_ENDPOINT_MAPPING.get(legacy_path, "/api/v1/llm/")

    def _track_legacy_usage(self, request: Request, user_info: Dict, process_time: float):
