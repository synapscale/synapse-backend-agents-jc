...
' --tree-filter '
    # Limpar README.md
    if [ -f README.md ]; then
        sed -i "s/AVNS_DDsc3wHcfGgbX_USTUt/YOUR_AIVEN_PASSWORD/g" README.md
        sed -i "s/doadmin:YOUR_AIVEN_PASSWORD/YOUR_DB_USER:YOUR_AIVEN_PASSWORD/g" README.md
    fi
    
    # Limpar documentação
    if [ -f "docs/GUIA_COMPLETO_SYNAPSCALE.md" ]; then
        sed -i "s/AVNS_DDsc3wHcfGgbX_USTUt/YOUR_AIVEN_PASSWORD/g" "docs/GUIA_COMPLETO_SYNAPSCALE.md"
        sed -i "s/doadmin:YOUR_AIVEN_PASSWORD/YOUR_DB_USER:YOUR_AIVEN_PASSWORD/g" "docs/GUIA_COMPLETO_SYNAPSCALE.md"
    fi
    
    # Limpar database.py
    if [ -f src/synapse/database.py ]; then
        sed -i "s/AVNS_DDsc3wHcfGgbX_USTUt/YOUR_AIVEN_PASSWORD/g" src/synapse/database.py
        sed -i "s/doadmin:YOUR_AIVEN_PASSWORD/YOUR_DB_USER:YOUR_AIVEN_PASSWORD/g" src/synapse/database.py
