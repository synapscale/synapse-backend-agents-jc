...
def test_main_router():
    ...
    try:
        ...
        marketplace_routes = [
            ...
            if hasattr(r, "path") and "/marketplace" in r.path
        ]
...
def test_app_routes():
    ...
    try:
        ...
        api_routes = [
            r for r in app.routes if hasattr(r, "path") and "/api/v1" in r.path
        ]
        print(f"   - Rotas da API v1: {len(api_routes)}")

        # Verificar rotas específicas
        auth_routes = [
            r for r in app.routes if hasattr(r, "path") and "/api/v1/auth" in r.path
        ]
        marketplace_routes = [
            r
            for r in app.routes
            if hasattr(r, "path") and "/api/v1/marketplace" in r.path
        ]
