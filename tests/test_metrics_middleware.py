...
def test_metrics_registry_isolation():
    ...
    metric_names = [metric._name for metric in CUSTOM_REGISTRY._collector_to_names.keys()]
    synapscale_metrics = [name for name in metric_names if name.startswith('synapscale_')]
    
    assert len(synapscale_metrics) > 0, "Devem existir métricas do SynapScale no registry personalizado"
