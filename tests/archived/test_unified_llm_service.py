...
class TestUnifiedLLMService:
    ...
    async def test_get_available_models_with_provider_filter(
        self, llm_service, mock_llm_model
    ):
    ...
    async def test_validate_model_provider_invalid(self, llm_service):
        ...
        # Execute method
        result = await llm_service.validate_model_provider(
            "invalid-model", "invalid-provider"
        )
    ...
    async def test_get_model_details_success(
        self, llm_service, mock_llm_model, sample_llm_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_model_by_provider_and_name_found(
        self, llm_service, mock_llm_model, sample_llm_data
    ):
...
class TestUnifiedLLMServiceIntegration:
    ...
    async def test_multiple_operations_sequence(
        self, llm_service_with_real_session, mock_llm_model
    ):
