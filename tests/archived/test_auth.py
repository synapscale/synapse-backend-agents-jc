...
class TestAuthentication:
    ...
    def test_login_valid_credentials(self, client: TestClient):
        ...
        login_data = {"username": unique_email, "password": "LoginPassword123!"}
    ...
    def test_refresh_token_endpoint(self, client: TestClient):
        ...
        client.post("/api/v1/auth/register", json=user_data)
        login_data = {"username": unique_email, "password": "RefreshPassword123!"}
...
class TestAuthorization:
    ...
    def test_user_can_access_own_data(self, client: TestClient, auth_headers):
        """Teste se usuário pode acessar seus próprios dados"""
        response = client.get("/api/v1/auth/me", headers=auth_headers)
        assert response.status_code == 200

    def test_user_cannot_access_admin_endpoints(self, client: TestClient, auth_headers):
...
class TestAuthenticationFlow:
    ...
    async def test_complete_auth_flow(self, async_client: AsyncClient):
        ...
        login_data = {"username": unique_email, "password": "FlowPassword123!"}
        ...
        if login_response.status_code in [200, 201]:
            login_json = login_response.json()
            # Check new response format with data wrapper
            has_access_token = (
                ("access_token" in login_json)
                or ("data" in login_json and "access_token" in login_json["data"])
                or ("token" in login_json)
            )
            assert has_access_token, f"No access token found in response: {login_json}"
        # 4. Acessar endpoint protegido com token
        if login_response.status_code in [200, 201]:
            login_json = login_response.json()
            # Extract token from new response format
            if "data" in login_json and "access_token" in login_json["data"]:
