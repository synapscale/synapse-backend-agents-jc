...
class TestWorkflowEndpoints:
    ...
    def test_create_workflow(self, client: TestClient, sample_workflow_data):
...
class TestFileEndpoints:
    ...
    def test_upload_file_endpoint_exists(self, client: TestClient):
...
class TestMarketplaceEndpoints:
    ...
    def test_list_components(self, client: TestClient):
        ...
        response = client.get("/api/v1/marketplace/components/")
        assert response.status_code in [200, 401]
    
    def test_get_template_by_id(self, client: TestClient):
        """Teste de obtenção de template por ID"""
        response = client.get("/api/v1/marketplace/templates/1")
        assert response.status_code in [200, 401, 404]
...
class TestAnalyticsEndpoints:
    ...
    def test_analytics_metrics(self, client: TestClient):
...
class TestLLMEndpoints:
    ...
    def test_llm_generate_endpoint(self, client: TestClient):
        ...
        response = client.post("/api/v1/llm/generate", json=generate_data)
...
class TestWorkspaceEndpoints:
    ...
    def test_create_workspace(self, client: TestClient, sample_workspace_data):
        """Teste de criação de workspace"""
        response = client.post("/api/v1/workspaces/", json=sample_workspace_data)
        assert response.status_code in [200, 201, 401]
...
class TestUserVariableEndpoints:
    ...
    def test_list_user_variables(self, client: TestClient):
        """Teste de listagem de variáveis"""
        response = client.get("/api/v1/user-variables/")
        assert response.status_code in [200, 401]
    
    def test_create_user_variable(self, client: TestClient):
        ...
        response = client.post("/api/v1/user-variables/", json=variable_data)
        assert response.status_code in [200, 201, 401]
