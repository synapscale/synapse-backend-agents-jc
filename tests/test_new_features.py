...
async def test_workflow_execute_and_list(
    async_client: AsyncClient, db_session, test_user, test_utils
):
    workflow = await test_utils.create_test_workflow(db_session, test_user)

    resp = await async_client.post(f"/api/v1/workflows/{workflow.id}/execute", json={})
    assert resp.status_code == 200
    data = resp.json()
    assert data["status"] == "pending"

    list_resp = await async_client.get(f"/api/v1/workflows/{workflow.id}/executions")
...
async def test_node_rating(
    ...
    resp = await async_client.post(
        f"/api/v1/nodes/{node.id}/rate", params={"rating": 4}
    )
    assert resp.status_code == 200
    data = resp.json()
