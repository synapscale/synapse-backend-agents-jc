...
def create_test_user(db: Session) -> User:
    ...
    try:
        ...
        if test_user:
            print(
                f"✅ Usuário de teste já existe: {test_user.email} (ID: {test_user.id})"
            )
            return test_user
...
def main():
    ...
    try:
        ...
        try:
            ...
            print(
                f"🔑 API Keys: {'✅ Configuradas' if setup_success else '❌ Problemas'}"
            )
            print(
                f"🧪 Testes: {'✅ Funcionando' if test_success else '❌ Com problemas'}"
            )
