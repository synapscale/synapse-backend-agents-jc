...
def install_dependencies():
    ...
    commands = [
        ...
        (
            ...
            "Instalando pytest-xdist para testes paralelos",
        ),
    ]
...
def run_unit_tests():
    ...
    cmd = ["python", "-m", "pytest", "tests/", "-m", "unit", "--tb=short", "-v"]
...
def run_metrics_tests():
    ...
    cmd = ["python", "-m", "pytest", "tests/", "-m", "metrics", "--tb=short", "-v"]
...
def run_parallel_tests():
    ...
    cmd = [
        ...
        "tests/",
        "-n",
        "auto",  # Usa todos os cores disponíveis
        "--tb=short",
        "-v",
        "--maxfail=5",
    ]
...
def setup_test_environment():
    ...
    if not env_test_path.exists():
        print(
            "❌ Arquivo .env.test não encontrado. Copie de .env.example e configure antes de rodar os testes."
        )
        sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Executar testes do SynapScale Backend"
    )
    parser.add_argument(
        "--type",
        choices=[
    ...
    parser.add_argument(
        "--install", action="store_true", help="Instalar dependências antes dos testes"
    )
    parser.add_argument(
        "--setup", action="store_true", help="Configurar ambiente de teste"
    )
    parser.add_argument(
        "--no-cov", action="store_true", help="Executar sem cobertura de código"
    )
