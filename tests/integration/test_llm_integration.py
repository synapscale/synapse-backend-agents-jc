...
async def test_llm_configuration():
    ...
    print(
        f"   Anthropic API Key: {'✅ Set' if settings.ANTHROPIC_API_KEY else '❌ Not set'}"
    )
...
async def test_token_counting():
    ...
    test_text = (
        "This is a test message to count tokens. It should work with all providers."
    )
...
async def test_text_generation():
    ...
    if (
        ...
        and settings.ANTHROPIC_API_KEY != "sua_chave_anthropic_aqui"
    ):
    ...
    for provider, model in providers_to_test:
        try:
            ...
            result = await llm_service.generate_text(
                prompt=test_prompt, provider=provider, model=model, max_tokens=100
            )
...
async def test_chat_completion():
    ...
    try:
        llm_service = get_llm_service()
        result = await llm_service.chat_completion(messages=messages, max_tokens=100)
            ...
            async with aiohttp.ClientSession() as session:
                if method == "GET":
                    async with session.get(f"{base_url}{endpoint}") as response:
                        status = (
                            "✅" if response.status == 200 else f"❌ {response.status}"
                        )
                        print(f"     {status}")
                elif method == "POST":
...
async def main():
    ...
    has_real_keys = any(
        [
            ...
            and settings.OPENAI_API_KEY != "sua_chave_openai_aqui",
            settings.ANTHROPIC_API_KEY
            and settings.ANTHROPIC_API_KEY != "sua_chave_anthropic_aqui",
            settings.GOOGLE_API_KEY
            and settings.GOOGLE_API_KEY != "sua_chave_google_aqui",
        ]
    )
