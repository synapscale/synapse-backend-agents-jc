...
class AuthenticationTester:
    ...
    def setup_authentication(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/auth/signup", json=register_data, timeout=10
            )
    ...
    def discover_endpoints(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"📊 {len([e for e in self.all_endpoints if e['is_public']])} endpoints públicos esperados"
                )
                print(
                    f"📊 {len([e for e in self.all_endpoints if not e['is_public']])} endpoints privados esperados"
                )
    ...
    def test_endpoint_with_auth(self, endpoint: Dict) -> Dict:
        ...
        try:
            if method == "GET":
                ...
                response = self.session.post(
                    full_url, json={"test": "data"}, headers=headers
                )
            elif method == "PUT":
        ...
        for endpoint in [e for e in self.all_endpoints if e["is_public"]]:
            ...
            if result.get("status") == "success":
                ...
                self.results["public_endpoints_broken"].append(
                    {"endpoint": endpoint_name, "result": result}
                )
        ...
        for endpoint in [e for e in self.all_endpoints if not e["is_public"]]:
            ...
            if result.get("status_code") in [401, 403]:
                ...
                self.results["authentication_bypassed"].append(
                    {"endpoint": endpoint_name, "status_code": result["status_code"]}
                )
                print(
                    f"🚨 {endpoint_name} - {result['status_code']} (BYPASS DE AUTENTICAÇÃO!)"
                )
            else:
                print(
                    f"⚠️  {endpoint_name} - {result.get('status_code', 'ERROR')} (erro)"
                )
    ...
    def print_final_report(self):
        ...
        print(
            f"   Protegidos corretamente: {self.results['private_endpoints_properly_protected']}"
        )
        ...
        if self.results["authentication_bypassed"]:
            ...
            print(
                f"   Endpoints com bypass de autenticação: {len(self.results['authentication_bypassed'])}"
            )
        ...
        if self.results["private_endpoints_tested"] > 0:
            protection_rate = (
                self.results["private_endpoints_properly_protected"]
                / self.results["private_endpoints_tested"]
            ) * 100
            print(f"\n🛡️  TAXA DE PROTEÇÃO: {protection_rate:.1f}%")

            if protection_rate < 100:
                print(
                    "🚨 FALHA CRÍTICA: Nem todos os endpoints privados estão protegidos!"
                )
            else:
                print("✅ Todos os endpoints privados estão protegidos")

        # Resumo final
        total_issues = len(self.results["authentication_bypassed"]) + len(
            self.results["public_endpoints_broken"]
        )
        if total_issues == 0:
...
def main():
    ...
    if not auth_success:
        print(
            "⚠️  Não foi possível configurar autenticação. Alguns testes podem falhar."
        )
