...
def mock_storage_manager():
    ...
    with patch("src.synapse.services.file_service.StorageManager") as mock:
        ...
        manager.save_file = AsyncMock(return_value={
        ...
        manager.generate_download_url = AsyncMock(return_value="http://test-url.com/download")
        yield manager
...
@pytest.mark.asyncio
async def test_upload_file(file_service, mock_db_session, sample_file, mock_storage_manager, mock_security_validator):
...
async def test_get_file(file_service, mock_db_session, sample_db_file):
    ...
    mock_db_session.execute.return_value.scalars.return_value.first.return_value = sample_db_file
...
@pytest.mark.asyncio
async def test_delete_file(file_service, mock_db_session, sample_db_file, mock_storage_manager):
    ...
    mock_storage_manager.delete_file.assert_called_once_with(
        category=sample_db_file.category,
        stored_name=sample_db_file.stored_name
    )
...
@pytest.mark.asyncio
async def test_generate_download_url(file_service, mock_db_session, sample_db_file, mock_storage_manager):
