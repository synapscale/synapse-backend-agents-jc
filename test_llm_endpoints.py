...
async def test_model_listing():
    ...
    try:
        ...
        print_success(f"Total: {total_models} modelos disponíveis (esperado: {total_count})")
        return True
...
async def test_error_handling():
    ...
    error_tests = [
        ...
        },
        {
            ...
            "params": {"prompt": "Hello", "provider": "openai", "model": "invalid_model"},
                    ...
                    if method == "GET":
                        async with session.get(f"{base_url}{endpoint}") as response:
                            if response.status == 200:
                                ...
                                print_warning(f"Precisa autenticação (status: {response.status})")
