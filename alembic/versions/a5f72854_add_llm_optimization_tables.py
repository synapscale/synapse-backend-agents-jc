...
def upgrade():
    ...
    if "llms" not in existing_tables:
        op.create_table(
            "llms",
            ...
            sa.Column(
                "cost_per_token_input", sa.Float(), nullable=False, server_default="0.0"
            ),
            sa.Column(
            ...
            sa.Column(
                "supports_function_calling", sa.Boolean(), server_default="false"
            ),
        ...
        op.create_index(
            "ix_llms_provider", "llms", ["provider"], schema="synapscale_db"
        )
        op.create_index("ix_llms_name", "llms", ["name"], schema="synapscale_db")
        op.create_index(
            "ix_llms_is_active", "llms", ["is_active"], schema="synapscale_db"
        )

    # 2. Relacionamento conversations <-> LLMs
    if "llms_conversations_turns" not in existing_tables:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["llm_id"], ["synapscale_db.llms.id"], ondelete="CASCADE"
            ),
            sa.PrimaryKeyConstraint("id"),
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_conversations_turns_conversation_id",
            "llms_conversations_turns",
            ["conversation_id"],
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_conversations_turns_llm_id",
            "llms_conversations_turns",
            ["llm_id"],
            schema="synapscale_db",
        )
    ...
    if "llms_usage_logs" not in existing_tables:
        op.create_table(
            "llms_usage_logs",
            sa.Column("id", sa.UUID(), nullable=False),
            ...
            sa.ForeignKeyConstraint(
                ["message_id"], ["synapscale_db.llms_messages.id"], ondelete="CASCADE"
            ),
            sa.ForeignKeyConstraint(
            ...
            sa.ForeignKeyConstraint(
                ["llm_id"], ["synapscale_db.llms.id"], ondelete="RESTRICT"
            ),
            sa.ForeignKeyConstraint(
                ["workspace_id"], ["synapscale_db.workspaces.id"], ondelete="SET NULL"
            ),
            sa.PrimaryKeyConstraint("id"),
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_usage_logs_user_id",
            "llms_usage_logs",
            ["user_id"],
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_usage_logs_conversation_id",
            "llms_usage_logs",
            ["conversation_id"],
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_usage_logs_llm_id",
            "llms_usage_logs",
            ["llm_id"],
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_usage_logs_created_at",
            "llms_usage_logs",
            ["created_at"],
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_usage_logs_workspace_id",
            "llms_usage_logs",
            ["workspace_id"],
            schema="synapscale_db",
        )
    ...
    if "billing_events" not in existing_tables:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ...
                ["synapscale_db.llms_usage_logs.id"],
                ondelete="SET NULL",
            ),
            sa.ForeignKeyConstraint(
                ["related_message_id"],
                ["synapscale_db.llms_messages.id"],
                ondelete="SET NULL",
            ),
            sa.PrimaryKeyConstraint("id"),
            schema="synapscale_db",
        ...
        op.create_index(
            "ix_billing_events_workspace_id",
            "billing_events",
            ["workspace_id"],
            schema="synapscale_db",
        )
    ...
    if "llms_message_feedbacks" not in existing_tables:
        op.create_table(
            ...
            sa.Column("user_id", sa.UUID(), nullable=False),
            sa.Column(
                "rating_type", sa.String(20), nullable=False
            ),  # thumbs_up, thumbs_down, star_rating
            sa.Column(
                "rating_value", sa.Integer(), nullable=True
            ),  # 1-5 para stars, 1/-1 para thumbs
            sa.Column("feedback_text", sa.Text(), nullable=True),
            sa.Column(
                "feedback_category", sa.String(50), nullable=True
            ),  # helpful, accurate, creative, etc
        ...
        op.create_index(
            "ix_llms_message_feedbacks_message_id",
            "llms_message_feedbacks",
            ["message_id"],
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_message_feedbacks_user_id",
            "llms_message_feedbacks",
            ["user_id"],
            schema="synapscale_db",
        )
        op.create_index(
            "ix_llms_message_feedbacks_rating_type",
            "llms_message_feedbacks",
            ["rating_type"],
            schema="synapscale_db",
        )

    # Inserir LLMs padrão apenas se a tabela foi criada agora
    if "llms" not in existing_tables:
        op.execute(
            """
