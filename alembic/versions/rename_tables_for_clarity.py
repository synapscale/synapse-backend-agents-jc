...
def upgrade():
    ...
    if "message_feedbacks" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.message_feedbacks RENAME TO llms_message_feedbacks"
        )

    if "conversations" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.conversations RENAME TO llms_conversations"
        )

    if "conversation_llms" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.conversation_llms RENAME TO llms_conversations_turns"
        )

    # Rename workflow-related tables only if they exist
    if "execution_metrics" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.execution_metrics RENAME TO workflow_execution_metrics"
        )

    if "execution_queue" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.execution_queue RENAME TO workflow_execution_queue"
        )
...
def downgrade():
    ...
    if "workflow_execution_queue" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.workflow_execution_queue RENAME TO execution_queue"
        )

    if "workflow_execution_metrics" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.workflow_execution_metrics RENAME TO execution_metrics"
        )

    # Revert LLM-related tables
    if "llms_conversations_turns" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.llms_conversations_turns RENAME TO conversation_llms"
        )

    if "llms_conversations" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.llms_conversations RENAME TO conversations"
        )

    if "llms_message_feedbacks" in existing_tables:
        op.execute(
            "ALTER TABLE synapscale_db.llms_message_feedbacks RENAME TO message_feedbacks"
        )
